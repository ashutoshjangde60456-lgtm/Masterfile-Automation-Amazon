...
def worksheet_used_cols(ws, header_rows=(1,), hard_cap=2048, empty_streak_stop=8):
    # NOTE: if ws.max_column is None for some edge case, guard it
    ws_max = ws.max_column if isinstance(ws.max_column, int) and ws.max_column > 0 else 1
    max_try = min(ws_max, hard_cap)
...
# safe output filename (keeps letters, numbers, space, _ . -)
...
def _patch_sheet_xml(sheet_xml_bytes: bytes, header_row: int, start_row: int, used_cols_final: int, block_2d: list) -> bytes:
    ...
    # 2) Remove mergeCells that intersect our data region
    ...
    # 3) Append dense rows (A..lastCol) using inlineStr
    ...
    # 6) Clear filterMode flag if present
...
with c1:
    # Any filename is OK; we only restrict file *type*
    masterfile_file = st.file_uploader("📄 Masterfile Template (.xlsx / .xlsm)", type=["xlsx", "xlsm"])
with c2:
    # CHANGE: allow onboarding .xlsm too (any filename is OK)
    onboarding_file = st.file_uploader("🧾 Onboarding (.xlsx / .xlsm)", type=["xlsx", "xlsm"])
...
with tab2:
    # Any filename is OK; we only restrict extension
    mapping_json_file = st.file_uploader("Or upload mapping.json (any name works)", type=["json"], key="mapping_file")
...
if go:
    ...
    # Determine extension & mime based on uploaded template name (any filename OK)
    ext = (Path(masterfile_file.name).suffix or ".xlsx").lower()
    if ext not in (".xlsx", ".xlsm"):
        # Safety: if someone uploads a weird name but valid file, default to .xlsx
        ext = ".xlsx"
    mime_map = {
    ...
    # ─────────────────────────────────────────────────────────────
    # CHANGE: Robust parse mapping JSON (paste OR upload), any filename
    # ─────────────────────────────────────────────────────────────
    try:
        if mapping_json_text and mapping_json_text.strip():
            mapping_raw = json.loads(mapping_json_text)
        elif mapping_json_file is not None:
            mapping_json_file.seek(0)
            mapping_raw = json.load(mapping_json_file)
        else:
            st.error("Please provide Mapping JSON either by **pasting** it or **uploading** a .json file.")
            st.markdown("</div>", unsafe_allow_html=True)
            st.stop()
    ...
    used_cols = worksheet_used_cols(
        ws_ro,
        header_rows=(MASTER_DISPLAY_ROW, MASTER_SECONDARY_ROW),
        hard_cap=2048,
        empty_streak_stop=8
    )
    ...
    for c, (disp, sec) in enumerate(zip(display_headers, secondary_headers), start=1):
        ...
        if resolved is not None:
            ...
            report_lines.append(f"- ✅ **{label_for_log}** ← `{used_alias}`")
        else:
    ...
    # Download — chosen base name + template extension
...
with st.expander("📘 How to use (step-by-step)", expanded=False):
    ...
    **Run**
    1) Upload the Masterfile (.xlsx/.xlsm) and the Onboarding (.xlsx/.xlsm)
    2) Paste/upload Mapping JSON (any filename works)
